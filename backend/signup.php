...
if ($check_result->num_rows == 0) {
    ...
    if ($account_type === 1 && !empty($organisation)) { 
        ...
        if ($organisation_result) {
            echo json_encode(['redirect' => '/login', 'message' => 'user and organization created']);
        } else {
            echo json_encode(['error' => 'error during creating user']);
        }
    } else {
        echo json_encode(['redirect' => '/login', 'message' => 'user created']);
    }
} else {
    echo json_encode(['error' => 'email exist already']);
}
